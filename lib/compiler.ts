...
export class ProjectCompiler implements ICompiler {
	...
	private emit(result: CompilationResult, callback: ts.WriteFileCallback) {
		...
		result.emitErrors += this.reportDiagnostics(emitOutput.diagnostics);
	...
	/**
	 * Reports the new diagnostics and return their number
	 *
	 * If a diagnostic was already encountered, ignores it. It snsures every diagnostic is unique.
	 *
	 * @param diagnostics Array of diagnostics to report if they are new
	 * @return Number of reported diagnostics
	 */
	private reportDiagnostics(diagnostics: ts.Diagnostic[]): number {
		let reportedCount: number = 0;
		for (const error of diagnostics) {
			if(this.project.output.diagnostic(error)) {
				reportedCount++;
			}
		}
