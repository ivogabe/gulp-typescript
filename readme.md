...
3. Execute `typings install` (Install `typings` with `npm install -g typings`).
4. Execute `git submodule update --init` to pull down the TypeScript compiler/services versions used in the test suite.
5. Ensure the gulp CLI is globally installed (`npm install -g gulp`).
6. Execute the tests: `gulp`.
